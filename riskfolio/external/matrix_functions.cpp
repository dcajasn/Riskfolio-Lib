 ...
 */
py::array_t<double> duplication_matrix(const int & n) {
    int cols = (n*(n+1))/2;
    int rows = n*n;
    int col;
 ...
 * @n size of symmetric matrix.
 */
py::array_t<double> duplication_elimination_matrix(const int &n) {
    int rows = (n*(n+1))/2;
    int cols = n*n;
    int row;
    arma::sp_mat out = sp_mat(rows, cols);
 ...
 * @n size of symmetric matrix.
 */
py::array_t<double> duplication_summation_matrix(const int &n) {
    arma::mat D;
    arma::mat L;
    D = carma::arr_to_mat(duplication_matrix(n));
    L = carma::arr_to_mat(duplication_elimination_matrix(n));
    arma::sp_mat D0;
    arma::sp_mat L0;
    arma::sp_mat S0;
    D0 = arma::sp_mat(D);
