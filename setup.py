...
if __name__ == "__main__":
    ...
    # PYTHON_REQUIRES[-1] takes the last character, which is "0" for ">=3.10",
    # so the guard compared against (3, 0) and never fired.
    _min_version = tuple(int(x) for x in PYTHON_REQUIRES.lstrip(">=").split("."))
    if sys.version_info[: len(_min_version)] < _min_version:
    ...
    eigen_unsupported_path = os.path.abspath(
        os.path.join(".", "lib", "eigen-3.4.0", "unsupported")
    )
    ...
    if WIN:
        external_module = Pybind11Extension(
            "riskfolio.external.functions",
            sources=sources,
            include_dirs=[
                numpy_include,
                eigen_path,
                eigen_core_path,
                eigen_unsupported_path,
                spectra_path,
                external_path,
                external_path,
            ],
    ...
    setup(
        ...
        project_urls={
            "Documentation": "https://portfoliooptimization.org",
